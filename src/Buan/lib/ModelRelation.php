...
class ModelRelation {
	...
	*/
	static public function define($relationship, $options=NULL, $relationRef=NULL, $manyToManyPartial=NULL) {
		...
		static $processedRelationships = array();
		...
		if(substr_count($relationship, ":")>1) {
			...
			self::$relationships[$mSource][$mTarget][$relationRef] = new ModelRelation(array(
				'modelSource'=>$mSource,
				'modelTarget'=>$mTarget,
			...
			if($mSource!=$mTarget) {
				...
				self::$relationships[$mSource][$mTarget][self::REF_CHILD] = new ModelRelation(array(
					'modelSource'=>$mSource,
					'modelTarget'=>$mTarget,
					'cardinality'=>ModelRelation::MANY_TO_MANY,
					'modelLink'=>$mLink,
		...
		if(!isset(self::$relationships[$m_model[0]][$m_model[1]][$relationRef])) {
			...
			if($m_model[0]==$m_model[1] && $cardinality==self::MANY_TO_ONE) {
				...
				self::$relationships[$m_model[1]][$m_model[0]][self::REF_CHILD] = new ModelRelation(array(
			...
			else if($m_model[0]==$m_model[1] && $cardinality==self::ONE_TO_MANY) {
				// Create the "M:1" relationship here (ref: self::REF_CHILD)
				self::$relationships[$m_model[1]][$m_model[0]][self::REF_CHILD] = new ModelRelation(array(
					'modelSource'=>$m_model[1],
					'modelTarget'=>$m_model[0],
			...
			self::$relationships[$m_model[0]][$m_model[1]][$relationRef] = new ModelRelation(array(
			...
			if($m_model[0]!=$m_model[1] && $manyToManyPartial===NULL) {
				self::$relationships[$m_model[1]][$m_model[0]][$relationRef] = new ModelRelation(array(
					'modelSource'=>$m_model[1],
					'modelTarget'=>$m_model[0],
					'cardinality'=>$revCardinality,
					'modelLink'=>$manyToManyPartial,
	...
	/*
	# @method ModelRelation|array getRelation( string $modelSource, [string $modelTarget, [string $relationRef]] )
	# $modelSource	= Name of the Model on the LEFT side of the relationship
	# $modelTarget	= Name of the Model on the RIGHT side of the relationship
	# $relationRef	= The specific relationship to return
	#
	...
	*/
	static public function getRelation($modelSource, $modelTarget=NULL, $relationRef=NULL) {
		...
		if($modelTarget===NULL) {
			$relations = array();
			$relationMap = isset(self::$relationships[$modelSource]) ? self::$relationships[$modelSource] : array();
	...
	*/
	public function getInverseRelation() {
		...
		else if(isset(self::$relationships[$modelTarget][$modelSource])) {
